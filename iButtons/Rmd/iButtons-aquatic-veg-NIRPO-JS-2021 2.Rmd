...
plot_type <- read.csv("../data/plot_type_long2.csv")
...
knitr::kable(head(iBtn_summary[, 1:8]), "simple", col.names = c("Date","Time","iBtn ID","Temp (\u00B0C)","Plot ID","Plot_type","Community","Sensor Type"))
```

```{r}
#write.csv(iBtn_summary, "../data/iBtn_summary_long_pivot2.xlsx") - uncomment to save new csv file to dir
```
...
avg_daily_temp <- iBtn_summary %>%
  group_by(Date, iBtn_ID, Plot_ID, Sensor_type, Plot_type, Community) %>% 
  summarize(avg_daily_temp = mean(temp_c, na.rm = T), .groups = "drop") %>% 
  mutate(across(where(is.numeric), ~ round(., 1)))

knitr::kable(head(avg_daily_temp[, 1:7]), "simple", col.names = c("Date","iBtn ID","Plot ID","Sensor Type","Plot Type","Community","Avg Daily Temp (\u00B0C)"))
```
...
sedi_moss_filter <- avg_daily_temp %>% 
    filter(Sensor_type == "Sediment", Plot_type == "Moss")
```
...
p <- ggplot(sedi_moss_filter, aes(x = Date, y = avg_daily_temp)) +
  ...
  labs(title = "Avg. Daily Temp at Sediment Surface in Moss Plots", x = NULL, y = "Temperature (\u00B0C)") +
...
sedi_aqforb_filter <- avg_daily_temp %>% 
    filter(Sensor_type == "Sediment", Plot_type == "Forb")
```
...
p <- ggplot(sedi_aqforb_filter, aes(x = Date, y = avg_daily_temp)) +
  ...
  facet_wrap(~Plot_ID, ncol = 4) + 
  labs(title = "Avg. Daily Temp at Sediment Surface in Forb Plots", x = NULL, y = "Temperature (\u00B0C)") +
  theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle=45, hjust = 1))
...
sedi_bare_filter <- avg_daily_temp %>% 
    filter(Sensor_type == "Sediment", Plot_type == "Sparse")
```
...
p <- ggplot(sedi_bare_filter, aes(x = Date, y = avg_daily_temp)) +
  ...
  facet_wrap(~Plot_ID, ncol = 4) + 
  labs(title = "Avg. Daily Temp at Sediment Surface in Sparse Plots", x = NULL, y = "Temperature (\u00B0C)") +
  theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle=45, hjust = 1))
...
```{r}
ggsave(here("iButtons/figures/", "adt-plot-facet.png"), plot = p.adt_plot_facet, width = 9, height = 3)
```

```{r}
ggsave(here("ibuttons/figures/","sedi-moss-facet.png"), plot = p.sedi_moss_facet, width = 8, height = 8, units = "in")
```
